...
import static org.mmocore.gameserver.geodata.GeoEngine.EAST;
import static org.mmocore.gameserver.geodata.GeoEngine.NORTH;
import static org.mmocore.gameserver.geodata.GeoEngine.NSWE_ALL;
import static org.mmocore.gameserver.geodata.GeoEngine.NSWE_NONE;
import static org.mmocore.gameserver.geodata.GeoEngine.SOUTH;
import static org.mmocore.gameserver.geodata.GeoEngine.WEST;
...
import org.mmocore.gameserver.geodata.PathFindBuffers.GeoNode;
import org.mmocore.gameserver.geodata.PathFindBuffers.PathFindBuffer;
...
public class PathFind
{
	...
	public static final List<Location> findPath(int x, int y, int z, int destX, int destY, int destZ, boolean isPlayable, int geoIndex)
	{
		...
		for(Location p : path)
			//targetRecorder.add(GeoEngine.g2w(p, geoIndex));
	...
	private static void pathClean(List<Location> path, int geoIndex)
	{
		...
		if (size2 > 2)
			...
			{
				Location three = path.get(j); // точка конца движения
				Location two = path.get(j - 1); // точка в середине, кандидат на вышибание
				Location one = path.get(j - 2); // точка начала движения
				boolean scv = GeoEngine.simpleCanMove(one.x, one.y, one.z, three.x, three.y, three.z, geoIndex);
				//System.out.println("two"+one.x+",:"+one.y+",:"+two.x+",:"+two.y+",:"+three.x+",:"+three.y+",:"+scv);
				if (GeoEngine.simpleCanMove(one.x, one.y, one.z, three.x, three.y, three.z, geoIndex))
				{
					path.remove(j - 1); // удаляем ее
					size2--; // отмечаем это в размере массива
					j = Math.max(2, j - 2); // сдвигаемся назад, FIXME: может я тут не совсем прав
				}
			}
		}		
		...
		if (size3 > 2)
		{
			Location three2 = path.get(0); // точка конца движения
			Location two2 = path.get(1); // точка в середине, кандидат на вышибание
			Location one2 = path.get(2); // точка начала движения
				if (GeoEngine.canMoveWithCollision(one2.x, one2.y, one2.z, three2.x, three2.y, three2.z, geoIndex))
				{
					path.remove(1); // удаляем ее
					size2--; // отмечаем это в размере массива
				}
		}*/
	...
	private static boolean IsPointInLine(Location p1, Location p2, Location p3)
	{
		...
		if((p1.x-p2.x == p2.x-p3.x)&&(p1.y-p2.y == p2.y-p3.y))
			return true;
		//if(((p1.x - p2.x) * (p1.y - p2.y) == (p2.x - p3.x) * (p2.y - p3.y)) && (p1.x - p2.x) * (p1.y - p2.y)!=0 && (p2.x - p3.x) * (p2.y - p3.y)!=0)
		return false;
	}
	...
	private List<Location> findPath(boolean isPlayable)
	{
	...
	private float traverseCost(GeoNode from, GeoNode n, boolean d)
	{
		...
		//	if(Math.abs(n.z - hNSWE[0]) > 16){ return 2f; }
